Public Class LaporanRekapitulasiSimpananPokok
    ...
    Sub showData()
        If cmb_anggota.SelectedIndex = 0 Then
            sql = "select anggota, to_char(now(), 'DD-MM-YYYY') as tglrekap, (select coalesce (sum(besar), 0) from qrekening where kategori = 'WAJIB' and idanggota = tblanggota.idanggota and tglrek between '" & dtp_mulai.Value.ToString("dd-MM-yyyy") & " 00:00' and '" & dtp_sampai.Value.ToString("dd-MM-yyyy") & "') as wajib, (select coalesce (sum(besar), 0) from qrekening where kategori = 'POKOK' and idanggota = tblanggota.idanggota and tglrek between '" & dtp_mulai.Value.ToString("dd-MM-yyyy") & " 00:00' and '" & dtp_sampai.Value.ToString("dd-MM-yyyy") & "') as pokok, (select coalesce(sum(besar), 0) from qrekening where kategori = 'SALDO AWAL' and idanggota = tblanggota.idanggota and tglrek between '" & dtp_mulai.Value.ToString("dd-MM-yyyy") & " 00:00' and '" & dtp_sampai.Value.ToString("dd-MM-yyyy") & "') as saldoawal from tblanggota where anggota ilike '%" & txt_search.Text & "%' and idanggota in (select idanggota from tblrekening group by idanggota)"
